...
# This build script doesn't call for sampling or resize any more.
...
def sample_prots(reserved_proteins, repre_num_threshold = 5, nodename = "TP", \
	...
	for rep_id in repre_prot_dict:
		# sample 1/10 of all representatives.
...
def build_pair2(reserved_proteins, keys, seqs, output_file):
	...
	with open(output_file, 'w') as fout:
		...
		invalid_2_count = 0
		too_long_count = 0
		all_cate_stat = {0: 0, 1: 0, 2: 0, 3: 0, 4: 0}
		random_dict = {0: 1, 1: 7, 2: 7, 3: 10, 4: 500}
		print(f"Init all_cate_stat dictionray: {json.dumps(all_cate_stat)}")
		...
		for i, pro1 in enumerate(all_keys):
		    for pro2 in all_keys[i+1:]:
		    	...
		    	if pair_count % 100000 == 0:
		    		print(f"Dealed pair count: {pair_count}")
		    	if pro1 == pro2:
		    		invalid_1_count += 1
		    		...
		    		if random.randint(0, random_dict[distance]) != 1:
		    			continue
		    		
		    		# Filter too long sequences.
		    		if len(thesequences[pro1]) > 500 or len(thesequences[pro2]) > 500:
		    			too_long_count += 1
		    			continue

		    		all_cate_stat[distance] += 1
		    		if all_cate_stat[distance] % 10000 == 0:
		    			print(f"Distance = {distance} reached {all_cate_stat[distance]}.")
		...
		print(f"There are {too_long_count} too long sequence inside the pair in total.")
		print(f"Final statistics of pair amount: {json.dumps(all_cate_stat)}")
...
if __name__ == "__main__":
	...
	train_keys, validate_keys, excluded_validate_keys = \
		sample_prots(reserved_proteins, repre_num_threshold = 5, nodename = "CL", 
			node_threshold = "1000002", theseed = 2024, if_filter = True)
